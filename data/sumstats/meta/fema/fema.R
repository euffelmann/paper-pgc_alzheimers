...
params_cc <- data.frame(
  ...
  filter(grepl("fema", sumstats_file))
...
params_proxy <- data.frame(
  ...
  filter(grepl("mother", sumstats_file) & grepl("main", sumstats_file))
...
outfile <- "fema"
out_dir <- here("data/sumstats/meta/fema")
scheme <- "STDERR"
...
for (casec in c("combined", "case_control", "proxy")) {
  for (anc in c("all", "eur", "afr", "eas", "sas", "amr")) {
    ...
    name <- paste0("fema_", casec, "_", anc)
    input <- paste0(casec, "/", name)
    neff_th <- 0.6
    n_sumstats_th <- 1
    
    if (!file.exists(here("data/sumstats/meta/fema", paste0(input, "_neff", neff_th, "_nsumstats", n_sumstats_th, ".txt.gz")))) {
      ...
      Rscript filter_sumstats.R ", here("data/sumstats/meta/fema", input), " ", neff_th, " ", n_sumstats_th, " 
      "
        ),
        directory = here("data/sumstats/meta/fema/"))
